"""MARCHLAND toy: BP-Lattice grid-density approximation. 1 agent ~ 10 men.
No attack/defense/morale stats. Class A constants identical for all humans;
sides differ only via B (gear), C (fatigue/belief from campaign), D (doctrine), E (drill error).
Modes: stochastic (seeded) and deterministic (hazard accumulation)."""
import numpy as np, json, sys

A = dict(lam0=0.0008, lamx=2.2, fat_amp=2.0, fat_melee=0.0035, fat_move=0.0006,
         fat_mud=4.0, fat_rec=0.0008, p_down=0.45, hes=16.0, horse_solid=3.0,
         w=dict(downs=3.2, rout=2.4, expose=0.9, behind=2.6, fat=1.0, banner=1.4, press=0.8))
DT = 2.0
...
class Battle:
    ...
    def tick(self):
        ...
        for s in (0,1):
            ...
            cnt.append(sm9(gC)); rtg.append(sm9(gR))
        ...
        if 'wall' in scn:
            wx, gates, dside = scn['wall']   # x, [(y0,y1),...], defending side
            ...
            # funnel: clamped stormers sidle along the wall toward the nearest gate
            ...
            # stormers inside halt and hold (carry the wall, don't parade off the map)
            deep = (self.side != dside) & (((self.advd<0)&(self.x < wx-12)) | ((self.advd>0)&(self.x > wx+12)))
            self._halt = deep
            # a breach is rubble, not rampart: defenders standing in a gate strip lose wall cover
    ...
    def run(self, ticks=1600):
        ...
        for s in (0,1):
            ...
            r[s] = dict(total=tot*10, rout=int((self.rout&m&self.alive).sum())*10, dead=(self.kpre[s]+self.kpost[s])*10,
